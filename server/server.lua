    ...
    if item == "bat" then
       MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
            if result[1].fivecoin >= 250 then
                ...
                PerformHttpRequest('https://canary.discord.com/api/webhooks/932743349750861934/TzvpKZE5g4mG-nSlcS67LWg2iDAcDrUohgrMFJulVtwu3fWBsDWAF3lBYwh1joDacB-C', function(err, text, headers) end, 'POST', json.encode({username = "Five-Boutique", content = xPlayer.getName() .. " a acheter " .. item}), { ['Content-Type'] = 'application/json' })
                cb(true)         
            else
                cb(false)
            end
...
if item == "rmodbugatti" then
    MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
         if result[1].fivecoin >= 1000 then
             local newpoint = result[1].fivecoin - 1000
               MySQL.Async.execute("UPDATE `users` SET `fivecoin`= '".. newpoint .."' WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function() end)
             PerformHttpRequest('https://canary.discord.com/api/webhooks/932743349750861934/TzvpKZE5g4mG-nSlcS67LWg2iDAcDrUohgrMFJulVtwu3fWBsDWAF3lBYwh1joDacB-C', function(err, text, headers) end, 'POST', json.encode({username = "", content = xPlayer.getName() .. " a acheter " .. item}), { ['Content-Type'] = 'application/json' })
             cb(true)         
         else
             cb(false)
         end
